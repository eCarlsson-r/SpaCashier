...
export const groupVoucherByTreatmentName = (data: VoucherData[]) => {
  return data.reduce(
    (
      acc: { [key: string]: { items: VoucherData[]; subAmount: number } },
      voucher,
    ) => {
      ...
      acc[key].subAmount += voucher.amount;
...
export const VoucherReportTemplate = ({
  variant,
  endDate,
  data,
}: {
  ...
  const voucherRecords = useMemo(
    () => groupVoucherByTreatmentName(data),
    [data],
  );

  // Calculate Grand Totals
  const grandTotal = Object.values(voucherRecords).reduce(
    (acc, curr) => ({
  ...
  return (
    <div className="p-3 bg-white min-h-screen">
      ...
      <table className="w-full">
        ...
        <tbody>
          {Object.entries(voucherRecords).map(([treatmentName, group]) => (
            <Fragment key={treatmentName}>
              ...
              {variant != "QTY" && (
                <tr className="font-bold italic bg-gray-50">
                  <td colSpan={3} className="text-right text-sm pr-3">
                    Subtotal
                  </td>
