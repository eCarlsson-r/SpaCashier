...
export const groupVoucherByType = (data: any[]) => {
  return data.reduce((acc, account) => {
    ...
    acc[key].subAmount += parseInt(account.balance);
    acc[key].current += parseInt(account.current);
    acc[key].previous += parseInt(account.previous);
    return acc;
  }, {});
};

export const ProfitLossTemplate = ({ startDate, endDate, data }: { startDate: string, endDate: string, data: any[] }) => {
    // Use useMemo so we don't re-calculate totals on every render
    const accountRecords = useMemo(() => groupVoucherByType(data), [data]);

    // Calculate Grand Totals
    const grandTotal = Object.values(accountRecords).reduce((acc: any, curr: any) => ({
        current: (curr.type=="income")?acc.current + curr.current:acc.current - curr.current,
        previous: (curr.type=="income")?acc.previous + curr.previous:acc.previous - curr.previous,
            ...
            <table className="w-full">
                <thead>
                    <tr className="bg-slate-50 border-y border-slate-200">
                       ...
                       <th className="py-2 px-2 text-sm font-semibold">Until This Month</th>
                    </tr>
                </thead>
                <tbody>
                    {Object.entries(accountRecords).map(([treatmentName, group]: [string, any]) => (
                    <Fragment key={treatmentName}>
                        {/* Payment Group Header */}
                        <tr className="bg-brown-50 font-bold">
                            <td colSpan={3} className="p-1">{treatmentName}</td>
                        </tr>
