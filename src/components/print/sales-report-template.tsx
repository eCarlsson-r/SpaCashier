...
export const groupSalesByPayment = (data: any[]) => {
  return data.reduce((acc, sales) => {
    const key = (sales.income.payments.length > 0)?sales.income.payments[0].description.split(" dengan ")[0]:"Uang Tunai";
    if (!acc[key]) {
      acc[key] = {
        items: [],
        subAmount: 0,
    ...
    const subtotal = parseInt(sales.subtotal, 10) || 0;
    const discount = parseInt(sales.discount, 10) || 0;
...
export const SalesReportTemplate = ({ startDate, endDate, branch, data }: { startDate: string, endDate: string, branch: string, data: any[] }) => {
    // Use useMemo so we don't re-calculate totals on every render
    const salesRecords = useMemo(() => groupSalesByPayment(data), [data]);

    // Calculate Grand Totals
    const grandTotal = Object.values(salesRecords).reduce((acc: any, curr: any) => ({
        ...
        <div className="p-3 bg-white min-h-screen">
            ...
            <div className="flex justify-between items-end mb-8 border-b-2 pb-4">
                <div>
                    ...
                    <p className="text-slate-500 uppercase font-semibold">Cabang: {branch}</p>
                </div>
            ...
            <table className="w-full">
                ...
                <tbody>
                    {Object.entries(salesRecords).map(([paymentAcc, group]: [string, any]) => (
                    <Fragment key={paymentAcc}>
                        {/* Payment Group Header */}
                        <tr className="bg-brown-50 font-bold">
                            <td colSpan={6} className="p-2">Pembayaran ke: {paymentAcc}</td>
                        </tr>
                        ...
                        <tr key={idx} className="border-b">
                            ...
                            <td className="text-sm">{formatIDR(item.subtotal - item.discount)}</td>
                        </tr>
                        ))}
