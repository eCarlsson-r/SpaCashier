...
export const PaymentModal = ({
  ...
  const totalPaid = useMemo(
    () => paymentsAdded.reduce((acc, p) => acc + p.amount, 0),
    [paymentsAdded],
  );
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="grid grid-cols-2 gap-4 p-3 bg-slate-50 rounded-lg border text-sm">
          <div>
            ...
            <span
              className={`font-bold ${remaining > 0 ? "text-red-600" : "text-gray-400"}`}
            >
          ...
          {currentMethod === "card" && (
            <div className="grid grid-cols-3 gap-2 p-3 border rounded bg-gray-50">
              <AppSelect
                ...
                placeholder="Type"
                onValueChange={(v) =>
                  setTempDetails((p) => ({ ...p, card_type: v }))
                }
                options={[
              ...
              <Input
                ...
                onChange={(e) =>
                  setTempDetails((p) => ({ ...p, card_number: e.target.value }))
                }
              />
            </div>
          )}
          ...
          {currentMethod === "ewallet" && (
            <div className="grid grid-cols-2 gap-2 p-3 border rounded bg-gray-50">
              <AppSelect
                ...
                placeholder="Type"
                onValueChange={(v) =>
                  setTempDetails((p) => ({ ...p, wallet_edc: v }))
                }
                options={[
          ...
          {currentMethod === "voucher" && (
            <div className="grid grid-cols-2 gap-2 p-3 border rounded bg-gray-50">
              <AppSelect
                ...
                placeholder="Type"
                onValueChange={(v) =>
                  setTempDetails((p) => ({ ...p, voucher_provider: v }))
                }
                options={[
        ...
        <div className="space-y-2 max-h-40 overflow-auto">
          {paymentsAdded.map((p, i) => (
            <div
              key={i}
              className="flex justify-between items-center p-2 border rounded text-xs bg-white"
            >
              {p.method === "card" && (
                <span>
                  {p.method.toUpperCase()} {p.details.card_type}{" "}
                  {p.details.card_edc}
                </span>
              )}
              {p.method === "ewallet" && (
                <span>
                  {p.method.toUpperCase()} {p.details.wallet_edc}
                </span>
              )}
              {p.method === "voucher" && (
                <span>
                  {p.method.toUpperCase()} {p.details.voucher_provider}
                </span>
              )}
              {p.method === "cash" && <span>{p.method.toUpperCase()}</span>}
              {p.method === "qr" && (
                <span>
                  {p.method.toUpperCase()} {p.details.qr_edc}
                </span>
              )}
