  ...
  const columns = useMemo(() => {
    ...
    return [
      ...baseColumns,
      {
        accessorKey: "period",
        header: "Period",
        cell: (info) => `${info.row.original.month} ${info.row.original.year}`,
      },
      { accessorKey: "count", header: "Count" },
      {
        accessorKey: "amount",
        header: "Amount",
        cell: (info) =>
          `Rp. ${new Intl.NumberFormat("id-ID").format(info.row.original.amount)},-`,
      },
    ];
  }, [selectedVariant]);
    ...
    if (selectedEndDate) {
      api
        ...
        .then((response) => {
          ...
          const flattened = response.data.flatMap(
            (treatment: { voucher: unknown; sales: unknown }) =>
            ...
            {selectedVariant == "QTY" && (
              <div className="mt-2">
                ...
                <AppSelect
                  ...
                  multiple={true}
                  onValueChange={(value) =>
                    setSelectedMetric(JSON.parse(value))
                  }
                  options={[
