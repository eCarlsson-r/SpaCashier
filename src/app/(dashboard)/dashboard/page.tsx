...
export default function Dashboard() {
  ...
  if (data?.monthly_income && data?.monthly_expense) {
    ...
    const hashmap: Record<
      number,
      { month: number; commision?: number; attendance?: number }
    > = {};
                  ...
                  { accessorKey: "attribute", header: "Payroll Attribute" },
                  {
                    ...
                    cell: ({ row }) =>
                      typeof row.original.value == "string"
                        ? row.original.value
                        : `Rp. ${formatCurr(row.original.value as number)}`,
                  },
                ]}
                data={payrollSummary}
              />
                    ...
                    cell: (info) => (
                      <div className="flex gap-3">
                        {parseInt(info.row.original.active_sessions || 0) +
                          parseInt(info.row.original.completed_sessions || 0)}
                      </div>
                    ),
                  },
