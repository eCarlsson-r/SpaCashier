...
const TotalRow = ({
  ...
  <div
    className={`flex justify-between py-1 ${highlight ? "border-t-2 border-gray-800 mt-2 pt-2" : ""}`}
  >
    <span className={`text-sm ${highlight ? "font-bold" : "text-gray-600"}`}>
      {label}
    </span>
    <span
      className={`text-sm ${highlight ? "font-bold text-lg" : "font-medium"}`}
    >
...
export default function SalesForm() {
  ...
  const [selectedBranch, setSelectedBranch] = useState<string>(
    activeUser?.employee?.branch_id.toString() || "",
  ); // This holds logo, address, etc.
  ...
  }, [subtotal, form, handleApplyDiscount]);
  ...
  const handleProcessSale = async (formData: FieldValues) => {
    try {
      const payload = {
  ...
  const [paymentsAdded, setPaymentsAdded] = useState<
    {
      method: string;
      amount: number;
      details: {
  ...
  const totalPaidSoFar = paymentsAdded.reduce(
    (acc, p) => acc + Number(p.amount),
    0,
  );
  // Change is only relevant if the total paid exceeds the grand total
  const changeAmount =
    totalPaidSoFar > grandTotal ? totalPaidSoFar - grandTotal : 0;
  ...
  const handleFinalize = async () => {
    try {
      const res = await api.put(`/sales/${currentSaleId}`, {
        id: currentSaleId,
