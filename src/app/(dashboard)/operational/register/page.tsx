...
export default function RegisterVoucherForm() {
  ...
  const generateVoucherPdf = (
    vouchers: { id: string; register_date: string }[],
    ...
    vouchers.forEach(
      (voucher: { id: string; register_date: string }, idx: number) => {
  ...
  const onRegister = (data: { quantity: number; treatment_id: string }) => {
    ...
    api
      ...
      .then(function (response) {
        ...
        const treatmentLabel = treatmentOptions.find(
          (t: { value: string; label: string }) =>
            t.value === data.treatment_id,
        )?.label;
        // Generate PDF string
  ...
  useEffect(() => {
    if (qty && treatment) {
      api
        .get(`/voucher?treatment=${treatment}`)
        .then((res) => {
          form.setValue("prefix", treatment);
          const voucherCode =
            "id" in res.data ? parseInt(res.data.id.split(treatment)[1]) : 0;
          form.setValue(
            "start",
            treatment + ("0000000000" + (voucherCode + 1)).slice(-6),
          );
          form.setValue(
            "end",
            treatment + ("0000000000" + (voucherCode + qty)).slice(-6),
          );
        })
        .catch((error) => {
          console.log("First set of vouchers", error);
          form.setValue("prefix", treatment);
      ...
      <div className="bg-white p-4 border rounded-sm shadow-sm">
        ...
        <div className="grid grid-cols-10 gap-4 mb-4">
          ...
          <div className="col-span-4 flex items-end gap-2">
            ...
            <span className="mb-2 text-gray-500 bg-gray-100 px-3 py-2 border rounded">
              until
            </span>
            <div className="flex-1">
              <FormField
          ...
          {pdfUrl ? (
            ...
            <div className="text-gray-400 flex flex-col items-center">
              ...
              <p className="mt-2 text-sm italic">
                PDF preview will appear here after registration.
              </p>
            </div>
          )}
        </div>
